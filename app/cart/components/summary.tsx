...
const Summary = () => {
  const cars = useCartStore((state: any) => state.cars || []); // Ensure default value is []
  
  const totalPrice = cars.reduce((prev: number, cur: TPopularCar) => {
        ...
        {cars.map((car: TPopularCar) => (
          ...
          >
            <div className="flex gap-x-4 items-center">
              ...
              <div>
                ...
                <div className="flex gap-x-1">
                  {useStarsCreate(car.stars_count).map((star, index) => (
            ...
            <div className="flex justify-between mb-4">
              ...
              <span className="text-secondary-500 font-semibold text-lg">${0}</span>
            </div>
          </li>
        ))}
      </ul>
